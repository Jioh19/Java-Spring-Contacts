...
class ContactsApplicationTests {
	...
	public void getContactByIdTest() throws Exception {
		...
		mockMvc.perform(request)
				...
				.andExpectAll(jsonPath("$.name").value(contacts[0].getName()),
						jsonPath("$.phoneNumber").value(contacts[0].getPhoneNumber()));
	...
	public void getAllContactsTest() throws Exception {
		...
		mockMvc.perform(request)
				.andExpectAll(status().isOk(),
						...
						jsonPath("$.size()").value(contacts.length),
						jsonPath("$.[?(@.phoneNumber == 6135342524 && @.name == \"Jon Snow\")]").exists(),
						jsonPath("$.[?(@.id == 2 && @.phoneNumber == 4145433332 && @.name == \"Tyrion Lannister\")]")
								.exists());
	}
	...
	public void validContactCreation() throws Exception {
		RequestBuilder request = MockMvcRequestBuilders.post("/contact")
				...
				.content(objectMapper.writeValueAsString(new Contact("Rayan", "123456789")));
